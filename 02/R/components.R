...
sidebar_menu_item <- function(
  ...
  if (length(subItems) == 0) {
    class_str <- if (isTRUE(selected)) {
      "nav-item sidebar-nav-item active d-flex gap-2"
    } else {
      "nav-item sidebar-nav-item d-flex gap-2"
    }
    ...
    return(
      tags$a(
        ...
        onClick = if (is.null(href) && !is.null(tabName)) {
          sprintf("switchTab('%s')", tabName)
        },
        icon,
        tags$span(text, badgeTag)
      )
    )
  } else {
    ...
    return(
      tags$div(
        ...
        tags$div(
          tags$a(
            class = "nav-item sidebar-nav-item w-100 d-flex gap-2",
...
sidebar_menu_sub_item <- function(
  ...
  class_str <- if (isTRUE(selected)) {
    "ps-4 nav-item sidebar-nav-item sidebar-nav-subitem active d-flex gap-2"
  } else {
    "ps-4 nav-item sidebar-nav-item sidebar-nav-subitem d-flex gap-2"
  }
...
theme <- bslib::bs_theme(
  version = 5,
  primary = "rgba(205, 230, 230, 1.00)",
  default = "#b1b1b1",
  bg = "hsla(0, 0%, 100%, 1.00)",
  fg = "hsla(0, 0%, 5%, 1.00)",
  base_font = "Inter",
  "sidebar-bg" = "hsla(0, 0%, 100%, 1.00)",
  "card-bg" = "rgba(242, 248, 248, 1.00)",
  "spacer" = "1rem",
  ...
  "card-border-radius" = "calc(var(--bs-border-radius) * 3)",
  "card-box-shadow" = "none",
  "card-padding" = "1rem",
  "border-color" = "var(--bs-gray-100)",
  "input-border-color" = "var(--bs-gray-100)",
  "list-group-bg" = "var(--bs-card-bg)"
) |>
